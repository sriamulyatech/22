...
const Franchise: React.FC = () => {
  ...
  const validateIndianMobile = (value: string) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === 'mobile') {
      ...
      if (sanitized.length === 10) validateIndianMobile(sanitized);
      else setMobileError('');
    } else if (name === 'email') {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!validateIndianMobile(formData.mobile)) return;
    setSubmitted(true);
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input
                        ...
                        autocomplete="name"
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-gold transition-all"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        autocomplete="tel"
                        pattern="[6-9]{1}[0-9]{9}"
                        className={`w-full bg-white/5 border ${mobileError ? 'border-red-500 ring-1 ring-red-500' : 'border-white/10'} rounded-xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-gold transition-all`}
                        placeholder="10-digit number"
                      />
                      {mobileError && <p className="text-red-500 text-[10px] font-bold mt-1">{mobileError}</p>}
                    </div>
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-gold uppercase tracking-[0.2em] mb-2">Email Address</label>
                    <input
                      type="email"
                      name="email"
                      required
                      ...
                      onChange={handleChange}
                      autocomplete="email"
                      className={`w-full bg-white/5 border ${emailError ? 'border-red-500 ring-1 ring-red-500' : 'border-white/10'} rounded-xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-gold transition-all`}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={handleChange}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-white/80 focus:outline-none focus:ring-2 focus:ring-gold appearance-none font-bold"
                    >
                      <option className="bg-[#001f3f]" value="5 - 10 Lakhs">5 - 10 Lakhs</option>
                      <option className="bg-[#001f3f]" value="10 - 20 Lakhs">10 - 20 Lakhs</option>
                      <option className="bg-[#001f3f]" value="20 Lakhs +">20 Lakhs +</option>
                    </select>
                  </div>
